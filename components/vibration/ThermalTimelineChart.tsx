...
type ThermalTimelineChartProps = {
  states: MissionState[];
...
function buildCyclePoints(
  ...
  thermal: Extract<ThermalCondition, { kind: "Cycle" }>
): ChartPoint[] {
...
export function ThermalTimelineChart({ states, intendedLife_h }: ThermalTimelineChartProps) {
  const series = useMemo(() => {
    ...
    states.forEach((state) => {
      ...
      if (state.thermal.kind === "Steady") {
        points.push({ time_h: start, temp_C: state.thermal.T_C });
        points.push({ time_h: end, temp_C: state.thermal.T_C });
      } else {
        points.push(...buildCyclePoints(start, end, state.thermal));
      }
      currentTime = end;
    });
    return points;
  }, [states, intendedLife_h]);
  ...
  return (
    <div className="rounded-xl border bg-white p-4">
      <div className="mb-2 flex items-center justify-between">
        <div>
          <div className="text-sm font-semibold">Thermal Timeline</div>
          <div className="text-xs text-gray-500">Temperature vs time with segment shading.</div>
        </div>
